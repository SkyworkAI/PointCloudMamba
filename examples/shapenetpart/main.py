...
def write_list_to_txt(lst, file_path):
    try:
        with open(file_path, 'w') as file:
            for item in lst:
                file.write(str(item) + '\n')
        print("列表内容已成功写入到文件:", file_path)
    except IOError:
        print("写入文件时出现错误")
def save_result2txt(coords, labels):
    save_dir = '/root/pred/'
    for i in range(len(coords)):
        save_txt_name = os.path.join(save_dir, "{}.txt".format(i))
        save_txts = []
        for coord, label in zip(coords[i], labels[i]):
            _txt = "{} {} {} {}".format(coord[0], coord[1], coord[2], label)
            save_txts.append(_txt)
        write_list_to_txt(save_txts, save_txt_name)
    return

@torch.no_grad()
def validate(model, val_loader, cfg, num_votes=0, data_transform=None, save_pred=False):

    pred_coords, pred_cls = [], []
    ...
    for idx, data in pbar:
        ...
        for v in range(num_votes+1):
            ...
            logits += model(data)
            if v == 0:
                for _pos in data['pos'].cpu().numpy():
                    pred_coords.append(_pos)
                for _cls in logits.max(dim=1)[1].cpu().numpy():
                    pred_cls.append(_cls)
    ...
    if save_pred:
        save_result2txt(pred_coords, pred_cls)
